 ...
 * Time: 12:01
 */
class SOAClient_my extends ClientAbstract
{
...
// 获取可发优惠券列表
    public function getContractorCouponList()
    {
        $this->_responseClass = 'service\message\contractor\getContractorCouponListResponse';
        $request = new getContractorCouponListRequest();
        $request->setContractorId(159);
        $request->setAuthToken('F1eqizNPOwJD8Iuo');
        $request->setCustomerId(33331);
        $header = new Header();
        $header->setSource(SourceEnum::CONTRACTOR);
        $header->setVersion(1);
        $header->setRoute('contractor.getContractorCouponList');
        $this->send(Message::pack($header, $request));
    }

    // 业务员发券
    public function receiveContractorCoupon()
    {
        $this->_responseClass = '';
        $request = new ReceiveContractorCouponRequest();
        $request->setContractorId(16);
        $request->setAuthToken('sXoM3R9tAYAT5UVk');
        $request->setCity(441800);
        $request->setCustomerId(1063);
        $request->setFrom([
            'coupon_list'=>[
                [
                    'rule_id'=>161,
        ...
        $header->setSource(SourceEnum::CONTRACTOR);
        $header->setVersion(1);
        $header->setRoute('contractor.receiveContractorCoupon');
        $this->send(Message::pack($header, $request));
    }

    // 货架供货商列表
    public function getShelvesWholesalerList()
    {
        $this->_responseClass = 'service\message\contractor\getShelvesWholesalerListResponse';
        $request = new getShelvesWholesalerListRequest();
        $request->setContractorId(16);
        $request->setAuthToken('kl7ZYKzJ408W4kW6');
        $request->setCustomerId(1063);
        $header = new Header();
        $header->setSource(SourceEnum::CONTRACTOR);
        $header->setVersion(1);
        $header->setRoute('contractor.getShelvesWholesalerList');
        $this->send(Message::pack($header, $request));
    }
    ...
    {
        $this->_responseClass = 'service\message\contractor\GetStoreStatisticalResponse';
        $request = new GetStoreInfoRequest();
        $request->setContractorId(16);
        $request->setAuthToken('kl7ZYKzJ408W4kW6');
        $request->setCustomerId(45392);
        $request->setCustomerStyle(1);
        $request->setOrderDay(15);
        $request->setGmvDay(30);
        $header = new Header();
        $header->setSource(SourceEnum::CONTRACTOR);
        $header->setVersion(1);
        $header->setRoute('contractor.getStoreStatistics');
//        print_r($_GET);
//        echo PHP_EOL;
//        print_r($_SERVER);
//        exit;
...
//        $this->getShelvesWholesalerList();
//        $this->receiveContractorCoupon();
//        $this->getContractorCouponList();
//        $this->searchStores();
